...
export default function ProgramDetail({ setPage }: { setPage: (page: Page) => void }) {
  ...
  const regionData = {
    Kodikulam: { 
      ...
      temp: '3.8°C',
      locations: [
        { id: 1, name: 'A-1 Community', dist: '2.1km', temp: '4.1°C', top: '33%', left: '25%' },
        { id: 2, name: 'Campus-2 Hub', dist: '3.5km', temp: '4.0°C', top: '65%', left: '70%' },
        { id: 3, name: 'Main Road Fridge', dist: '0.8km', temp: '3.9°C', top: '15%', left: '50%' }
      ]
    },
    Othakadai: { 
      ngos: 8, 
      ...
      temp: '4.1°C',
      locations: [
        { id: 4, name: 'Othakadai Center', dist: '1.2km', temp: '4.2°C', top: '40%', left: '40%' },
        { id: 5, name: 'Lake Side Hub', dist: '4.1km', temp: '4.0°C', top: '75%', left: '20%' }
      ]
    }
  };
            ...
            <div className="w-full md:w-1/4 flex flex-col items-end gap-4 text-right">
               <div className="relative w-full aspect-square md:aspect-[3/4] bg-white/5 rounded-[40px] border border-white/10 p-6 backdrop-blur-sm shadow-2xl overflow-hidden group">
                  ...
                  <motion.img 
                    ...
                    src="src/assets/images/regenerated_image_1778587352875.jpg" 
                    alt="Map" 
      ...
      <section className="py-20 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-12">
            ...
            <div className="md:col-span-2 relative aspect-[16/9] bg-slate-50 rounded-[32px] overflow-hidden border border-slate-200">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                <button 
                  ...
                  className="w-10 h-10 bg-white rounded-xl shadow-lg flex items-center justify-center text-slate-600 hover:text-primary-600 transition-all active:scale-95"
                >
              ...
              {/* Location Select */}
              <div className="absolute top-4 right-4 max-w-[200px] w-full">
                <div className="bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl p-3 shadow-xl flex items-center justify-between">
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase ml-1 tracking-widest">Active Hub</p>
                    <div className="text-[10px] font-black text-slate-900 bg-transparent border-none focus:ring-0 cursor-default p-0 h-auto">
                      {region} Hub
                    </div>
                  </div>
              ...
              {filteredLocations.map((loc) => (
                ...
                >
                  <div className="bg-white/95 backdrop-blur shadow-2xl p-2.5 rounded-2xl border border-slate-100 flex items-center gap-3">
                    <div className="w-8 h-8 bg-primary-600 rounded-xl flex items-center justify-center text-white">
                      <MapPin size={16} />
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-900 leading-none mb-1">{loc.name}</p>
                      <p className="text-[9px] text-primary-600 font-bold uppercase tracking-tighter">{loc.dist} | {loc.temp}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-[#051409] rounded-[40px] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl flex flex-col transform hover:translate-y-[-4px] transition-all duration-500 group/vitals">
          ...
          <div className="absolute inset-0 z-0 opacity-[0.15] blur-sm pointer-events-none group-hover/vitals:scale-105 transition-transform duration-1000">
            <img 
              src="src/assets/images/fridge_bg.jpg" 
              alt="" 
